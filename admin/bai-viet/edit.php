...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $kiemTraTrung = $pdo->prepare(
        'SELECT id
         FROM bai_viet
         WHERE (tieu_de = :tieu_de OR slug = :slug)
           AND id <> :id
         LIMIT 1'
    );
    ...
    if (empty($loi)) {
        try {
            ...
            if ($trangThai === 'xuat_ban') {
                $ngayDang = $ngayDangCu ?: date('Y-m-d H:i:s');
            } else {
                $ngayDang = null;
            }
            ...
            $loi[] = 'Không thể cập nhật bài viết: '
                . $e->getMessage();
        }
    }
}
                ...
                >
                    <div>
                        ...
                        <p class="text-muted mb-0">
                            Cập nhật nội dung câu chuyện đặc sản.
                        </p>
                    </div>
