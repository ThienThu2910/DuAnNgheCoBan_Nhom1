...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {    
    ...
    $slug = ($slug === '') ? taoSlug($tenCoSo) : taoSlug($slug);    
    ...
    $kiemTraTrung = $pdo->prepare('SELECT id FROM co_so_san_xuat WHERE ten_co_so = :ten_co_so OR slug = :slug LIMIT 1');  
    $kiemTraTrung->execute(['ten_co_so' => $tenCoSo, 'slug' => $slug]);    
    if ($kiemTraTrung->fetch()) $loi[] = 'Tên cơ sở hoặc đường dẫn đã tồn tại.';    

    $tenHinhAnh = null; $duongDanAnhMoi = null;
    if (isset($_FILES['hinh_anh']) && $_FILES['hinh_anh']['error'] !== UPLOAD_ERR_NO_FILE) {    
        $file = $_FILES['hinh_anh'];
        if ($file['error'] !== UPLOAD_ERR_OK) { $loi[] = 'Có lỗi xảy ra khi tải hình ảnh.'; }
    ...
    if (empty($loi)) {    
        try {    
            ...
            $stmt = $pdo->prepare('INSERT INTO co_so_san_xuat (ten_co_so, slug, dia_chi, so_dien_thoai, email, mo_ta, hinh_anh, vi_do, kinh_do, google_maps_url, trang_thai) VALUES (:ten_co_so, :slug, :dia_chi, :so_dien_thoai, :email, :mo_ta, :hinh_anh, :vi_do, :kinh_do, :google_maps_url, :trang_thai)');
            $stmt->execute([    
                'ten_co_so' => $tenCoSo, 'slug' => $slug, 'dia_chi' => $diaChi,
                'so_dien_thoai' => $soDienThoai !== '' ? $soDienThoai : null,
                'email' => $email !== '' ? $email : null,
                'mo_ta' => $moTa !== '' ? $moTa : null,
                'hinh_anh' => $tenHinhAnh, 'vi_do' => $viDo, 'kinh_do' => $kinhDo,
                'google_maps_url' => $googleMapsUrl !== '' ? $googleMapsUrl : null,
                'trang_thai' => $trangThai    
            ]);

            $coSoId = (int)$pdo->lastInsertId();
            ...
            $pdo->commit();
            $_SESSION['success'] = 'Thêm cơ sở sản xuất thành công.';
            header('Location: index.php');    
            exit;    
        } catch (Throwable $e) {    
            if ($pdo->inTransaction()) $pdo->rollBack();  
            if ($duongDanAnhMoi !== null && is_file($duongDanAnhMoi)) unlink($duongDanAnhMoi);    
            $loi[] = 'Không thể thêm cơ sở: ' . $e->getMessage();    
        }    
    }    
}    
?>
        ...
        <div class="card border-0 shadow-sm">    
            <div class="card-body p-4">    
                <div class="d-flex justify-content-between align-items-center mb-4">    
                    <h1 class="h3 mb-0">Thêm cơ sở sản xuất</h1>    
                    ...
                    <div class="row g-4">    
                        <div class="col-lg-8">    
                            <div class="mb-3">    
                                ...
                                <input type="text" id="ten_co_so" name="ten_co_so" class="form-control" value="<?= htmlspecialchars($tenCoSo) ?>" required autofocus>    
                            </div>  
                            <div class="mb-3">    
                                <label for="slug" class="form-label">Đường dẫn</label>    
                                <input type="text" id="slug" name="slug" class="form-control" value="<?= htmlspecialchars($slug) ?>" placeholder="Để trống để tự tạo">    
                            </div>  
                            <div class="mb-3">    
                        ...
                        <div class="col-lg-4">    
                            ...
                            <img id="imagePreview" src="" alt="Xem trước hình ảnh" class="mb-4">  
                            ...
                            <button type="submit" class="btn btn-success w-100">Lưu cơ sở</button>    
                        </div>    
                    </div>    
                </form>    
            </div>    
